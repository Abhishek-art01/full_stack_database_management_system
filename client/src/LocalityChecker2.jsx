...
export default function LocalityManager() {
    ...
    // ... inside LocalityManager component
    ...
    const fetchData = async (pageNo = 1) => {
        setLoading(true);
        try {
            const res = await fetch(`http://127.0.0.1:8000/api/localities/?page=${pageNo}`);

            if (!res.ok) throw new Error('Network response was not ok');

            const data = await res.json();

            console.log("Server Response:", data); // Check console to see what you actually got!

            // CHECK: Is the data in the new 'pagination' format?
            if (data.pagination && data.results) {
                // Yes, use the new format
                setLocalities(data.results);
                setTotalPages(data.pagination.total_pages);
                setPage(data.pagination.current_page);

                // Safe stat counting
                const pendingCount = data.results.filter(i => i.status === 'Pending').length;
                setStats({ pending: pendingCount });
            }
            // CHECK: Is the data just a plain list (Old Format)?
            else if (Array.isArray(data)) {
                console.warn("Backend is still sending a plain list. Pagination disabled.");
                setLocalities(data);
                ...
                console.error("Unknown data format received");
            }
    ...
    // Reload data whenever 'page' state changes
    useEffect(() => {
        fetchData(page);
    }, [page]);
    ...
    const filteredList = localities.filter(l =>
        // The || "" part converts null to an empty string so it doesn't crash
                                ...
                                <tbody>
                                    {loading ? (
                                        ...
                                        <tr key={loc.id}>
                                            ...
                                            <td>{loc.address}</td>
                                            <td>
                                                <span className={`status-badge status-${(loc.status || "pending").toLowerCase()}`}>
                                                    {loc.status || "Pending"}
                                                </span>
                                            </td>
                                            <td>
